...
const texts = computed(() => locale.value.startsWith('zh') ? {
  ...
  backendDownDesc: (error: string) => `请检查 ai-gateway-service :18080 是否已启动，并确认 /v1/admin 代理可访问。错误：${error}`,
  tabBasic: '基础配置',
  tabQuota: '队列配额',
  tabDocs: '文档',
  tabObserve: '队列观测',
  ...
  backendDownDesc: (error: string) => `Check that ai-gateway-service :18080 is running and /v1/admin is proxied correctly. Error: ${error}`,
  tabBasic: 'Basic',
