...
const texts = computed(() => locale.value.startsWith('zh') ? {
  ...
  apiDown: '队列配额接口不可用',
  apiDownDesc: (error: string) => `请检查 ai-gateway-service :18080 和 /ai-gateway/v1/admin/tenant-rate-limits 代理。错误：${error}`,
  retry: '重试',
  tenant: '租户',
  model: '模型',
  path: '路径',
  ...
  apiDownDesc: (error: string) => `Check ai-gateway-service :18080 and the /ai-gateway/v1/admin/tenant-rate-limits proxy. Error: ${error}`,
  retry: 'Retry',
  tenant: 'Tenant',
  model: 'Model',
  path: 'Path',
